...
export async function GET(request: NextRequest) {
  try {
    ...
    const mockData = {
      engagement: {
        totalEngagedUsers: Math.floor(userCount.count * 0.65),
        ...
        features: [
          {
            ...
            adopters: Math.floor(userCount.count * 0.45),
            adoptionRate: 0.45,
            timeToFirstUse: 1200,
            usageFrequency: 2.3,
            retentionRate: 0.78,
          ...
          },
          {
            ...
            totalUsers: userCount.count,
            adopters: Math.floor(userCount.count * 0.15),
            adoptionRate: 0.15,
            timeToFirstUse: 800,
            usageFrequency: 1.8,
            retentionRate: 0.92,
          ...
          },
          {
            ...
            totalUsers: userCount.count,
            adopters: Math.floor(userCount.count * 0.68),
            adoptionRate: 0.68,
            timeToFirstUse: 2400,
            usageFrequency: 1.2,
            retentionRate: 0.85,
        ...
        newFeatures: [
          {
            ...
            adopters: Math.floor(userCount.count * 0.23),
      ...
      userSegmentation: {
        demographics: {
          userTypes: [
            { type: 'student', count: Math.floor(userCount.count * 0.75), percentage: 0.75 },
            { type: 'counselor', count: Math.floor(userCount.count * 0.15), percentage: 0.15 },
            { type: 'parent', count: Math.floor(userCount.count * 0.08), percentage: 0.08 },
            { type: 'administrator', count: Math.floor(userCount.count * 0.02), percentage: 0.02 },
          ],
          categories: [
            { category: 'underrepresented', count: Math.floor(userCount.count * 0.35), percentage: 0.35 },
            { category: 'low_income', count: Math.floor(userCount.count * 0.28), percentage: 0.28 },
            { category: 'first_generation', count: Math.floor(userCount.count * 0.22), percentage: 0.22 },
            { category: 'general', count: Math.floor(userCount.count * 0.15), percentage: 0.15 },
          ],
          academicLevels: [
            { level: 'high_school', count: Math.floor(userCount.count * 0.45), percentage: 0.45 },
            { level: 'undergraduate', count: Math.floor(userCount.count * 0.35), percentage: 0.35 },
            { level: 'graduate', count: Math.floor(userCount.count * 0.15), percentage: 0.15 },
            { level: 'other', count: Math.floor(userCount.count * 0.05), percentage: 0.05 },
          ],
        },
        behavior: {
          powerUsers: Math.floor(userCount.count * 0.15),
          casualUsers: Math.floor(userCount.count * 0.60),
          trialUsers: Math.floor(userCount.count * 0.20),
          churned: Math.floor(userCount.count * 0.05),
        },
        accessibility: {
          accessibilityUsers: Math.floor(userCount.count * 0.15),
          screenReaderUsers: Math.floor(userCount.count * 0.08),
          keyboardNavigationUsers: Math.floor(userCount.count * 0.12),
          highContrastUsers: Math.floor(userCount.count * 0.10),
          textToSpeechUsers: Math.floor(userCount.count * 0.06),
        },
      },
